...
# ------------------------------------------------------------------ registro
def append_to_array(text, open_end, item):
    """Inserta item antes del ] que cierra el array abierto en open_end."""
    depth, i = 1, open_end
    while depth:
        depth += (text[i] == "[") - (text[i] == "]")
        i += 1
    ...
    while text[j - 1] in " \n\t":
        j -= 1
    sep = "" if text[j - 1] in ",[" else ","
    return text[:j] + sep + item + text[j:]
...
def register(title, description, read_time):
    ...
    if f'"{SLUG}"' not in s and f"'{SLUG}'" not in s:
        # tres formas segun el repo: blogPosts = ["slug", ...],
        # BLOG_POSTS: {slug, date}[] = [...] (trulyfreeqr) o
        # articles = ["slug", ...] con lo nuevo al final (trulyfreemortgage)
        m = re.search(r"(const\s+blogPosts\s*=\s*\[)", s)
        a = re.search(r"const\s+articles\s*=\s*\[", s)
        if m:
            s = s[:m.end()] + f'\n    "{SLUG}",' + s[m.end():]
        elif a:
            s = append_to_array(s, a.end(), f'\n    "{SLUG}",')
        else:
    ...
    if f'"{SLUG}"' not in b and f"'{SLUG}'" not in b:
        ...
        if a and not m:
            ...
            bi.write_text(append_to_array(b, a.end(), entry))
