...
const steps = [
  ...
  ['6. Estimate a refinance break-even manually', 'The Scenario Comparison view does not compute refinance closing costs — take the monthly savings from step 5 and divide your expected closing costs by that number to estimate the break-even month. Generally, if rates drop 1%, refinancing is worth investigating.'],
  ['7. Test a middle ground with Extra Monthly Payment', 'Switch to the standard calculator, set up the 30-year loan, and try different amounts in the "Payoff Acceleration" field to see how much extra you would need to pay to approach a 15-year payoff timeline. You might find that an extra $300 per month cuts your term from 30 to about 20 years.'],
  ['8. Compare the two full amortization schedules one at a time', 'The standard calculator shows one full amortization schedule at a time (there is no side-by-side or downloadable version). Run the 30-year numbers, note the balance at month 60, then switch the term to 15 years and note the balance at the same month to see the equity difference.'],
];
